...
namespace ExampleMod.Content.Projectiles
{
	...
	/// Make sure to check out <see cref="ExampleCloneWeapon" />, which fires this projectile; it itself is a cloned version 的 Meowmere.
	/// </summary>
	public class ExampleCloneProjectile : ModProjectile
	{
		public override void SetDefaults() {
			...
			// the Meowmere Projectile's SetDefault stats (例如 projectile.friendly and projectile.penetrate) on to our projectile,
			// so we don't have to go in到 source and copy the stats ourselves. It saves a lot of time and looks much cleaner;
			// if you're going to copy the stats of a projectile, use CloneDefaults().

			Projectile.CloneDefaults(ProjectileID.Meowmere);

			// 要 further the Cloning process, we can also copy the ai of any given projectile using AIType, since we want
			...
			// This 可以 done by modifying projectile.penetrate
			Projectile.penetrate += 3;
		}

		// While there are several different ways to change how our projectile 可能have differently, lets make it so
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 4; i++) {
				...
				// 生成 a new projectile 与 newly rotated velocity, belonging 到 original projectile owner. The new projectile will inherit the spawning source of this projectile.
		...
		// Now, using CloneDefaults() and aiType doesn't copy EVERY aspect 的 projectile. In Vanilla, several other methods
		// are used to generate different effects that aren't included in AI. 对于 case 的 Meowmere projectile, since the
		// ricochet sound is not included 在 AI, we must add it ourselves:
		public override bool OnTileCollide(Vector2 oldVelocity) {
			...
			// Hover 在 method below in VS to see its summary.
