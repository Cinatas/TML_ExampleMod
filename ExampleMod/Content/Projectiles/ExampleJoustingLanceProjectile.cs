...
namespace ExampleMod.Content.Projectiles
{
	public class ExampleJoustingLanceProjectile : ModProjectile
	{
		public override void SetStaticDefaults() {
			...
			// This will make sure the velocity 的 projectile will always be the shoot speed set 在 item.
			// Since the velocity 的 projectile affects how far out the jousting lance will spawn, we want the
		...
		public override void SetDefaults() {
			...
			// width and height do not affect the collision 的 Jousting Lance because we calculate that separately (see Colliding() below)
			Projectile.width = 25;
			...
			Projectile.alpha = 255; // The transparency 的 projectile, 255 for completely transparent. Our projectile will fade in (see the AI() below).
			...
			Projectile.scale = 1f; // The scale 的 projectile. This only effects the drawing and the width 的 collision.
		...
		public override void AI() {
			Player owner = Main.player[Projectile.owner]; // 获取 the owner 的 projectile.
			Projectile.direction = owner.direction; // Direction 将 -1 when facing left and +1 when facing right. 
			owner.heldProj = Projectile.whoAmI; // 设置 the owner's held projectile to this projectile. heldProj is used so th在 projectile 将 killed when the player drops or swap items.
			...
			// Scale factor (0.34f) means the last 34% 的 animation 将 used for retracting.
			...
			// 如果 the Jousting Lance is 不再 being used, kill the projectile.
			...
			float tipDist = 98 + extension * extendDist - retraction * retractDist; // If your Jousting Lance is larger or smaller than the standard size, it is recommended to change the shoot speed 的 item instead of this value.

			Vector2 center = owner.RotatedRelativePoint(owner.MountedCenter); // 获取 the center 的 owner. This accounts 对于 player being shifted up or down while riding a mount, sitting in a chair, etc.
			Projectile.Center = center; // 设置 the center 的 projectile 到 center 的 owner. Projectile.Center is now actually the tip 的 Jousting Lance.
			Projectile.position += Projectile.velocity * tipDist; // The projectile velocity contains the orientation 的 lance, multiply it by the tipDist to positi在 tip.

			// 设置 the rotation 的 projectile.
			...
			// What this means in this context is th在 speed value 将 closer to positive 1 if the player is moving 在 same direction as the directi在 lance was shot.
			// 示例： if the lance is shot up and 到 right, the value here 将 closer to 1 if the player is also moving up and 到 right.
			...
			if (playerVelocity > minimumDustVelocity && movementInLanceDirection > 0.8f) {
				// chance 对于 dust to spawn. The actual chance (see below) is 1/dustChance. We make the chance higher the faster the player is moving by making the denominator smaller.
				int dustChance = 8;
				...
				// 生成 the dusts based 在 dustChance. The dusts are spawned 在 tip 的 Jousting Lance.
		...
		public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) {
			// This will increase or decrease the knockback 的 Jousting Lance depending on how fast the player is moving.
			modifiers.Knockback *= Main.player[Projectile.owner].velocity.Length() / 7f;

			// This will increase or decrease the damage 的 Jousting Lance depending on how fast the player is moving.
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
			float rotationFactor = Projectile.rotation + (float)Math.PI / 4f; // The rotation 的 Jousting Lance.
			float scaleFactor = 95f; // How far back the hit-line 将 从 tip 的 Jousting Lance. You will need to modify this if you have a longer or shorter Jousting Lance. Vanilla uses 95f
			...
			// This Rectangle is the width and height 的 Jousting Lance's hitbox 即 used 对于 first step of collision.
			// 你 will need to modify the last two numbers if you have a bigger or smaller Jousting Lance.
			// Vanilla uses (0, 0, 300, 300) which 即 quite large 对于 size 的 Jousting Lance.
			// size doesn't matter too much because this rectangle is only a basic check 对于 collision (the hit-line is much more important).
			Rectangle lanceHitboxBounds = new Rectangle(0, 0, 300, 300);

			// 设置 the position 的 large rectangle.
			...
			// 这是 the back 的 hit-line with Projectile.Center being the tip 的 Jousting Lance.
			Vector2 hitLineEnd = Projectile.Center + rotationFactor.ToRotationVector2() * scaleFactor;

			// following is for debugging the size 的 hit line. This will allow you to easily see where it starts and ends.
			...
			// 首先 check that our large rectangle intersects 与 target hitbox.
			// Then we check to see if a line 从 tip 的 Jousting Lance 到 "end" 的 lance intersects 与 target hitbox.
		...
		// 我们 need to draw the projectile manually. If you don't include this, the Jousting Lance will 不 aligned 与 player.
		public override bool PreDraw(ref Color lightColor) {

			// SpriteEffects change which directi在 sprite is drawn.
			...
			// This will make it so the bottom 的 sprite is correctly facing down when shot 到 right.
			...
			// position 的 sprite. Not subtracting Main.player[Projectile.owner].gfxOffY will cause the sprite to bounce when walking up blocks.
			...
			// following is for debugging the size 的 collision rectangle. Set this 到 same size as the one you have in Colliding().
			// Rectangle lanceHitboxBounds = new Rectangle(0, 0, 300, 300);
