...
namespace ExampleMod.Content.Projectiles
{
	public class ExampleGolfBallProjectile : ModProjectile
	{
		...
		public override void SetDefaults() {
			Projectile.netImportant = true; // 指示 that this 弹幕 将 synced to a joining 玩家 (默认情况下, any projectiles active before the 玩家 joins (besides pets) are not synced over).
			Projectile.width = 7; // The 宽度 的 弹幕's hitbox.
			Projectile.height = 7; // The 高度 的 弹幕's hitbox.
			Projectile.friendly = true; // 设置ting this to anything other than 真 causes an 索引 out of bounds 错误.
