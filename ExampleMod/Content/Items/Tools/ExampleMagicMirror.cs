...
{
	// Magic Mirror is one 的 only vanilla items that does its action somewhere other than the start of its animation, 即 why we use code in UseStyle NOT UseItem.
	...
	internal class ExampleMagicMirror : ExampleItem
	{
		...
		public override string Texture => $"Terraria/Images/Item_{ItemID.IceMirror}"; // Copies the texture 对于 Ice Mirror, make your own texture if need be.
		...
		// 使用Style is called each frame th在 item is being actively used.
		public override void UseStyle(Player player, Rectangle heldItemFrame) {
			// Each frame, make some dust
			if (Main.rand.NextBool()) {
				Dust.NewDust(player.position, player.width, player.height, DustID.MagicMirror, 0f, 0f, 150, Color.White, 1.1f); // 使 dust 从 player's position and copies the hitbox of which the dust may spawn. Change these arguments 如果需要.
			}
			...
			if (player.itemTime == 0) {
				...
				// This code runs once halfway through the useTime 的 Item. You'll notice with magic mirrors you are still holding the item for a little bit after you've teleported.

				// 使 dust 70 times for a cool effect.
				for (int d = 0; d < 70; d++) {
				...
				// 使 dust 70 times for a cool effect. This dust is the dust 在 destination.
				for (int d = 0; d < 70; d++) {
