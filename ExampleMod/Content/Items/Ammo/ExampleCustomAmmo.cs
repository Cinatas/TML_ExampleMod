...
namespace ExampleMod.Content.Items.Ammo
{
	...
	public class ExampleCustomAmmo : ModItem
	{
		...
		public override void SetDefaults() {
			...
			Item.damage = 8; // The damage for projectiles isn't actually 8, it actually is the damage combined 与 projectile and the item together
			Item.DamageType = DamageClass.Ranged; // What type of damage does this ammo affect?

			Item.maxStack = Item.CommonMaxStack; // The maximum number of items that 可以 contained within a single stack
			...
			Item.value = Item.sellPrice(0, 0, 1, 0); // Item price in copper coins (可以 converted with Item.sellPrice/Item.buyPrice)
			Item.rare = ItemRarityID.Yellow; // The color th在 item's name 将 in-game.
			...
			Item.ammo = Item.type; // 重要. The first item in an ammo class sets the AmmoID to its type
		}
