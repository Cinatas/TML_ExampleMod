...
{
	// 这是 the item used to summon a boss, in this case the modded Minion Boss from Example Mod. For vanilla boss summons, see comments in SetStaticDefaults
	public class MinionBossSummonItem : ModItem
	{
		...
		public override bool? UseItem(Player player) {
			if (player.whoAmI == Main.myPlayer) {
				// 如果 the player using the item is the client
				...
				if (Main.netMode != NetmodeID.MultiplayerClient) {
					// 如果 the player is not in multiplayer, spawn directly
					NPC.SpawnOnPlayer(player.whoAmI, type);
				}
				else {
					// 如果 the player is in multiplayer, request a spawn
