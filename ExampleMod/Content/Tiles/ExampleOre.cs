...
namespace ExampleMod.Content.Tiles
{
	public class ExampleOre : ModTile
	{
		public override void SetStaticDefaults() {
			...
			Main.tileSpelunker[Type] = true; // The tile 将 affected by spelunker highlighting
			Main.tileOreFinderPriority[Type] = 410; // Metal Detector value, see https://terraria.wiki.gg/wiki/Metal_Detector
			Main.tileShine2[Type] = true; // 修改 the draw color slightly.
		...
		// 示例 of how to enable the Biome Sight buff to highlight this tile. Biome Sight is technically intended to show "infected" tiles, so this example is purely for demonstration purposes.
	...
	// 示例OreSystem contains code related to spawning ExampleOre. It contains both spawning ore during world generation, seen in ModifyWorldGenTasks, and spawning ore after defeating a boss, seen in BlessWorldWithExampleOre and MinionBossBody.OnKill.
	public class ExampleOreSystem : ModSystem
	{
		...
		public void BlessWorldWithExampleOre() {
			...
			// 参见 https://github.com/tModLoader/tModLoader/wiki/World-Generation/#long-running-tasks f或更多 information.
			ThreadPool.QueueUserWorkItem(_ => {
				...
				// 100 controls how many splotches of ore are spawned in到 world, scaled by world size. For comparison, the first 3 times altars are smashed about 275, 190, or 120 splotches 的 respective hardmode ores are spawned. 
				...
				for (int iteration = 0; iteration < splotches; iteration++) {
					// Find a point 在 lower half 的 rock layer but above the underworld depth.
					int i = WorldGen.genRand.Next(100, Main.maxTilesX - 100);
					int j = WorldGen.genRand.Next(highestY, Main.UnderworldLayer);

					// OreRunner will spawn ExampleOre in splotches. OnKill only runs 在 server or single player, so it is safe to run world generation code.
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight) {
			...
			if (ShiniesIndex != -1) {
				...
				// 示例OrePass is a class seen bellow
	...
	public class ExampleOrePass : GenPass
	{
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			// progress.Message is the message shown 到 user while the following code is running.
			// Try to make your message clear. You 可以 a little bit clever, but make sure it is descriptive enough for troubleshooting purposes.
			progress.Message = ExampleOreSystem.ExampleOrePassMessage.Value;

			// Ores are quite simple, we simply use a for loop and the WorldGen.TileRunner to place splotches 的 specified Tile 在 world.
			// "6E-05" is "scientific notation". It simply means 0.00006 but in some ways is easier to read.
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-05); k++) {
				// inside of this for loop corresponds to one single splotch of our Ore.
				// 首先, we randomly choose any coordinate 在 world by choosing a random x and y value.
				int x = WorldGen.genRand.Next(0, Main.maxTilesX);

				// WorldGen.worldSurfaceLow is actually the highest surface tile. In practice you might want to use WorldGen.rockLayer or other WorldGen values.
				int y = WorldGen.genRand.Next((int)GenVars.worldSurfaceLow, Main.maxTilesY);

				// Then, we call WorldGen.TileRunner with random "strength" and random "steps", 以及 as the Tile we wish to place.
				...
				// Alternately, we could check the tile already present 在 coordinate we are interested.
				// Wrapping WorldGen.TileRunner 在 following condition would make the ore only generate in Snow.
