...
namespace ExampleMod.Content.Tiles
{
	...
	public class ExampleTrap : ModTile
	{
		public override void SetStaticDefaults() {
			...
			// These 2 AddMapEntry and GetMapOption show off multiple 地图 Entries per 图格. 删除 GetMapOption and all but 1 的se for your own ModTile if you don't actually need it.
		...
		// Because this 图格 does not use a TileObjectData, and consequently does not have "real" 图格 styles, the correct 图格 style 值 can't be determined automatically. This means th在 correct 项 won't automatically 放下, so we must use GetItemDrops to calculate the 图格 style to determine the 项 放下. 
		public override IEnumerable<Item> GetItemDrops(int i, int j) {
			Tile t = Main.tile[i, j];
			int style = t.TileFrameY / 18;
			// It 可以 useful to share a single 图格 with multiple styles.
		...
		// PlaceInWorld is needed to facilitate styles and alternates since this 图格 doesn't use a TileObjectData. Placing 左 and 右 based on 玩家 方向 is usually done 在 TileObjectData, but the specifics of that don't work for how we want this 图格 to work. 
		...
		public override void HitWire(int i, int j) {
			...
			if (style == 0) {
				...
				if (Wiring.CheckMech(i, j, 60)) {
					spawnPosition = new Vector2(i * 16 + 8 + 0 * horizontalDirection, j * 16 + 9 + 0 * verticalDirection); // The extra numbers here 帮助 中心 the 弹幕 生成 位置 if you need to.

					// 在 a real mod you 应该 spawning projectiles that are both hostile and friendly to do 伤害 to both players and NPC, as Terraria traps do.
