...
namespace ExampleMod.Content.Tiles
{
	public class ExampleCustomFramingTile : ModTile
	{
		public override void SetStaticDefaults() {
			...
			//This is so that snow blocks try to connect to this 图格
			//Our custom framing code will make our 图格 have custom merging with snow, but we also need to specify this so the merging happens 在 snow 方块 side 以及
		...
		public override void PostTileFrame(int i, int j, int up, int down, int left, int right, int upLeft, int upRight, int downLeft, int downRight) {
			//For every even Y 坐标, we will 偏移 the 图格's vertical 帧 by the 大小 的 sheet so the 图格's 帧 ends up using the alternate 版本 在 duplicated sheet below
			if (j % 2 == 0) {
