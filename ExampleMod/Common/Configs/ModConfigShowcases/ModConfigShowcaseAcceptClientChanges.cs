...
namespace ExampleMod.Common.Configs.ModConfigShowcases
{
	...
	// 默认情况下 changes that don't require a reload 将 accepted, but modders can use AcceptClientChanges to further limit this behavior.
	// 使用 the AcceptClientChanges method to determine if the clients changes to this config 将 accepted and relayed to all other clients.
	public class ModConfigShowcaseAcceptClientChanges : ModConfig
	{
		...
		// This LocalizedText 将 used for a rejection message. Since it is static it won't be part 的 config shown in-game.
		...
		public override bool AcceptClientChanges(ModConfig pendingConfig, int whoAmI, ref NetworkText message) {
			...
			if (Main.dayTime && ((ModConfigShowcaseAcceptClientChanges)pendingConfig).OnlyChangeableDuringNight != OnlyChangeableDuringNight) {
				// The NetworkText class ensures that messages are shown to clients 在 client's selected language. The NetworkText.FromKey and LocalizedText.ToNetworkText methods 可以 used to create a NetworkText object from a translation key.
				...
				return false; // 返回 false to reject the changes
			}
			...
			// if a remote client 应该 treated as the host.
			// The Shorter Respawn mod, 例如, uses the user permissions system of HEROs Mod (a mod 即 intended for
			...
			// Accept the changes. This is the default behavior 的 AcceptClientChanges method.
			return true;
		}
	}
}
