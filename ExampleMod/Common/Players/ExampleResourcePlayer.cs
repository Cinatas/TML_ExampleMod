...
namespace ExampleMod.Common.Players
{
	public class ExampleResourcePlayer : ModPlayer
	{
		...
		public int exampleResourceMax; // 缓冲区 变量 即 used to 重置 最大 资源 to default 值 in ResetDefaults().
		...
		// Here are additional things you might need to implement if you intend to make a custom 资源:
		// - Multiplayer Syncing: The current example doesn't require MP code, but pretty much any additional functionality will require this. ModPlayer.SendClientChanges and CopyClientState 将 necessary, 以及 as SyncPlayer if you 允许 the 用户 to increase exampleResourceMax.
		// - 保存/加载 permanent changes to max 资源: You'll need to implement 保存/加载 to remember increases to your exampleResourceMax cap.
		...
		// We need this to ensure that regeneration rate and 最大 amount are 重置 to default values after increasing when conditions are 不再 satisfied (e.g. we unequip an 饰品 that increases our 资源)
