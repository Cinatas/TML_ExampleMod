...
namespace ExampleMod.Common.Players
{
	public class ExampleLuckPlayer : ModPlayer
	{
		public override void ModifyLuck(ref float luck) { // 修改Luck is what you'll normally use for any modded content that wants to modify luck.
			...
			// As the above code runs 每次 luck is calculated, and `hardMode` is accessible on both 客户端 and 服务器, we don't need to worry about multiplayer syncing.
			// If you have some code which relies on 客户端 side calculations, you will need to 同步 the variables to calculate luck correctly 在 服务器.
		}

		public override bool PreModifyLuck(ref float luck) { // PreModifyLuck is useful if you want to modify any vanilla luck values or want to 防止 vanilla luck calculations from happening.
			...
			return true; // PreModifyLuck returns 真 默认情况下, but you can also 返回 假 if you want to 防止 vanilla luck calculations from happening at all.
		}
	}
}
