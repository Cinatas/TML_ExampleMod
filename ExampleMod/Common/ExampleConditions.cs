...
namespace ExampleMod.Common
{
	...
	// 在使用的地方创建新的 Condition 并没有什么问题，如 ExampleNPCShop 和 ExamplePerson 中所示，
	// 但是将多次使用的 Condition 放在一个中心位置以避免拼写错误和其他错误是一个好主意。
	// 将 Condition 存储在字段中还可以公开这些条件以便更容易地实现跨模组兼容性。
	// 有关使用 Condition 类的更多信息，请参阅 https://github.com/tModLoader/tModLoader/wiki/Conditions
