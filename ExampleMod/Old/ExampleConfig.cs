...
{
	// This 文件 contains 2 real ModConfigs (and also a bunch of fake ModConfigs showcasing various ideas). One is set to ConfigScope.ServerSide and the other ConfigScope.ClientSide
	...
	// DONT use static members anywhere in this 类 (except for an automatically assigned 字段 named 实例 与 same 类型 as the ModConfig 类, if you'd rather write "MyConfigClass.实例" instead of "ModContent.GetInstance<MyConfigClass>()"), tModLoader maintains several instances of ModConfig classes which will not work well with static properties or fields.
	...
	public class ExampleConfigServer : ModConfig
	{
		...
		// We will use attributes to annotate our fields or properties so tModLoader can properly 处理 them.
		...
		// We must use the DefaultValue attribute instead of 设置 the 值 normally:
		[DefaultValue(true)]
...
#pragma warning restore CS0169
		...
		// You'll notice this next one is a 属性 instead of a 字段. That works too.
		...
		/*public override bool NeedsReload(ModConfig pendingConfig)
		{
			...
			return defaultDecision || otherLogic; // 重新加载 needed if either 条件 is met.
		}*/
	}
	...
	public class ExampleConfigClient : ModConfig
	{
		...
		public override void OnChanged() {
			...
			// We maintain both ExampleUI.visible and ShowCoinUI as 分离 values so ShowCoinUI can act as a default while ExampleUI.visible can change within a play 会话.
