...
namespace ExampleMod
{
	...
	// This example will teach the most commonly sought after 效果: "How to do X if the 玩家 has Y?"
	...
	// We can change Y to other conditions 以及. 例如, you could give the 玩家 the 效果 by having a "药水" ModItem give a ModBuff that sets the ModPlayer 变量 in ModBuff.更新
	// Another example 将 an 护甲 set 效果. Simply use the ModItem.UpdateArmorSet hook 

	// Below you will see the ModPlayer 类, and below that 将 another 类 called SimpleAccessory 对于 饰品 both 在 same 文件 for your reading convenience. This 饰品 will give our 效果 to our ModPlayer. 

	// This is the ModPlayer 类. Make note 的 classname, 即 SimpleModPlayer, since we 将 using this 在 饰品 项 below.
	public class SimpleModPlayer : ModPlayer
	{
		...
		// 重置Effects is used to 重置 effects back 到ir default 值. Terraria resets all effects every 帧 back to defaults so we will follow this design. (You might think to set a 变量 when an 项 is equipped and unassign the 值 when the 项 in unequipped, but Terraria is not designed that way.)
		public override void ResetEffects() {
			FrostBurnSummon = false;
		}

		// Here we use a "hook" to actually let our frostBurnSummon status take 效果. This hook is called anytime a 玩家 owned 弹幕 hits an 敌人. 
		public override void OnHitNPCWithProj(Projectile proj, NPC target, int damage, float knockback, bool crit) {
			// frostBurnSummon, as its 名称 suggests, applies frostBurn to 敌人 NPC but only for Summon projectiles.
			// In this if statement we check several conditions. We first check to make sure the 弹幕 that hit the NPC is either a 仆从 弹幕 or a 弹幕 that minions shoot.
			// We then check that frostBurnSummon is set to 真. The last check for not noEnchantments is because some projectiles don't 允许 enchantments and we want to honor that restriction.
	...
	{
		// Assigning multiple EquipType/动画 textures is easily done.
