...
namespace ExampleMod.Items
{
	internal class ExampleDrawTooltips : ModItem
	{
		...
		private Vector2 boxSize; // 存储 the size of our tooltip box
		...
		public override bool PreDrawTooltip(ReadOnlyCollection<TooltipLine> lines, ref int x, ref int y) {
			...
			// 计算 our width 对于 box, which 将 the width 的 longest text, plus some padding. This code takes into account Snippets and character widths.
			int widthForBox = texts.Max(t => (int)ChatManager.GetStringSize(Main.fontMouseText, t, Vector2.One).X) + paddingForBox * 2;
			// 计算 our height 对于 box, which 将 the sum 的 text heights, plus some padding
		...
		public override bool PreDrawTooltipLine(DrawableTooltipLine line, ref int yOffset) {
			if (!line.oneDropLogo) {
				...
				// Let's draw the item name centered so it's 在 middle, and let's add a form of separator
				...
				// If our line text equals our item name, this is our tooltip line 对于 item name
				...
				// This 可能 important, in case some mod does a lot of custom work and removes the standard tooltip
				// For tooltip layers, check the documentation for TooltipLine
				{
					...
					// which would place the START 的 string 在 center, so we subtract half 的 line width to center it completely
					...
					// yOffset affects the offset 即 added every next line, so this will cause the line to come after the separator to be drawn slightly lower
