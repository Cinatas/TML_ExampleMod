...
namespace ExampleMod.Items
{
	public class Infinity : ModItem
	{
		...
		public override void SetDefaults() {
			...
			item.expert = true;
			item.expertOnly = true; // 使 it so the item's accessory effects only work in expert mode
		}
		...
		public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI) {
			...
			// We redraw the item's sprite 4 times, 每次 shifted 2 pixels on each direction, using Main.DiscoColor to give it the color changing effect
			for (int i = 0; i < 4; i++) {
	...
	public class InfinityGlobalItem : GlobalItem
	{
		...
		// Replenishes the mana 的 player has soon as they need some, by exactly the amount they need, and stops the mana flower from triggering.
